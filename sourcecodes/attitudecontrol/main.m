%% Spacecraft Attitude Control with CV-STEM, ExpControl, PID, and Hinf
tStart = tic;
clear all
close all
syms t
...
%rng(688) % Let Nsim = 1 for the TAC plots
rng(9468) % Let Nsim = 60 for the TAC plots
%% Constants
Nsim = 60;
Ncon = 4;
n = 6; % #states
...
for nsim = 1:Nsim
    ...
    for k = 1:N
        ...
        for c = 1:Ncon
            ...
            elseif c == 2
                u = ExpController(t,X);
            elseif c == 3
                u = pid_controller(t,X);
            elseif c == 4
                [u,Phinf,Qphinf,cvx_statushinf] = Hinfinity(X,Xd,Qphinf);
            else
    ...
    qdhis = qd(t_his);
    dqddthis = dqd_dt(t_his);
    if Nsim == 1
        save('data/plots_data0.mat','t_his','X_his','ueff_his','mse_hisQ','Ncon','N')
    end
    samples = 150;
